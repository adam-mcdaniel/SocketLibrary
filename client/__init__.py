...
class Client():
    def __init__(self, serverip, serverport, workers=range(0, 1)):
    ...
    def run(self,worker):
        while True:
            if worker == 0:
                ...
                list(map(lambda i: print(str(i[0])), list(self.new_data.values())))
    ...
    def send(self, data):
        self.s.sendto(str.encode(str(data)),self.server)
